 ...
 * 
 * CHANGELOG:
 * - 2026-01-15: HOTFIX - Reverted PERF-2847 changes causing SEV-1 outage (rendering timeouts, blocked user access)
...
export const renderingConfig: RenderingConfig = {
  // File handling limits
  // HOTFIX 2026-01-15: Restored to 500MB - PERF-2847 reduction caused users to lose access to saved work
  maxFileSizeMB: 500,
  supportedFormats: ['psd', 'psb', 'tiff', 'png', 'jpeg', 'raw'],
  
  // Timeout configuration
  // HOTFIX 2026-01-15: Restored to original values - PERF-2847 reduction caused render timeouts
  renderTimeoutMs: 120000,  // 2 minutes - required for large PSD files with many layers
  exportTimeoutMs: 90000,   // 90 seconds - adequate for format conversion
  syncTimeoutMs: 120000,    // 2 minutes - required for large file sync on slower connections
  
  // Concurrency settings
  // HOTFIX 2026-01-15: Restored to 10 concurrent jobs - PERF-2847 reduction caused queue backlog
 ...
 * 
 * HOTFIX 2026-01-15: Fixed enterprise tier to use explicit values instead of
 * referencing base config, which caused issues when base config was modified.
 */
export function getConfigForTier(tier: 'free' | 'pro' | 'enterprise'): Partial<RenderingConfig> {
  const tierOverrides: Record<string, Partial<RenderingConfig>> = {
    ...
    pro: {
      maxFileSizeMB: 200,  // Pro tier: 200MB limit
      maxConcurrentJobs: 3,
      enableGpuRendering: true,
    },
    enterprise: {
      // Enterprise tier: highest limits with explicit values
      maxFileSizeMB: 500,  // FIXED: Explicit 500MB for enterprise (was incorrectly referencing base config)
      maxConcurrentJobs: 10,  // FIXED: Explicit value for enterprise
      enableGpuRendering: true,
      enableBatchOptimization: true,
    },
  };
