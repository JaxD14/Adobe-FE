...
describe('Rendering Configuration', () => {
  describe('File Size Limits', () => {
    ...
    it('should reject files over the limit', () => {
      // Testing against full enterprise limit (500MB)
...
describe('File Validation', () => {
  ...
  it('should validate file size against config', () => {
    ...
    // Enterprise limit is 500MB, so 200MB files should be valid
    const mediumFile = createMockFile({ sizeMB: 200 });
    expect(validateFile(mediumFile).valid).toBe(true);
    
    // Files over the enterprise limit should be rejected
...
describe('Processing Time Estimation', () => {
  ...
  it('should warn when estimated time exceeds timeout', () => {
    ...
    // 500MB, 200 layers, 16K resolution will exceed 300s timeout
    ...
    const estimate = estimateProcessingTime(extremeFile);
    
    // Files this large should exceed the timeout
